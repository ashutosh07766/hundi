/** Shared no-network fixtures for the claude-brain test suite: an in-memory
 ...
 * a fake Anthropic client that returns a scripted text response, and a
 * minimal but correctly-signed PurchaseMandate. */
...
import type { AnthropicClient } from '../claude-brain.js'
...
/** Returns a fake AnthropicClient whose single `messages.create` call always
 * responds with `text` as the sole text content block — enough to exercise
 * `askClaude`'s parse path without ever reaching the network. */
export function fakeAnthropic(text: string): AnthropicClient {
  return {
    messages: {
      async create() {
...
export function pickResponse(chosen_sku: string, reason = 'good fit'): string {
  return JSON.stringify({ chosen_sku, reason })
}
