...
async function main() {
  ...
  if (!Array.isArray(feed) || feed.length === 0) throw new Error(`no catalog for ${merchant} — onboard it first`)
  ...
  const tools = new HttpBuyerTools({ storeUrl: 'http://127.0.0.1:8796', facilitatorUrl: FAC, pollTimeoutMs: 120000, pollIntervalMs: 2000 })
  const m = await registerMandate({ facilitatorUrl: FAC, dashboardToken: TOKEN, goal, ceiling_paise: ceilingPaise, approval_threshold_paise: ceilingPaise, merchants: [merchant], expires_at: Math.floor(Date.now() / 1000) + 3600 })
  console.log(`\nGOAL: "${goal}"  ·  store: ${merchant}  ·  cap ₹${ceilingPaise / 100}\n`)
  const p = await runLlmPurchase({ goal: { query: goal, ceiling_paise: ceilingPaise, threshold_paise: ceilingPaise }, mandate: { intent: m.intent, agent: m.agentKeyPair }, config: cfg }, tools)
