 ...
 *
 * Side-effect-free on import — `runCli` only acts when called. This is what
 * lets `cli.ts` (the workspace bin) and any external wrapper that bundles
 * this package under a different binary name (e.g. the published `hundi`
 * npm package) both call `runCli` exactly once each, with no risk of a
 * second, unwanted invocation firing as an import side effect.
 */
...
/** Parses argv (without the `node`/script entries) and runs the requested
 * command, returning the process exit code. Callers own invoking this
 * exactly once and applying the returned code — this function never calls
 * `process.exit` itself. */
